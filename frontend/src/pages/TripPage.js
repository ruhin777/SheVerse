...
const MyTripCard = ({ trip, selectedTrip, requests, fetchRequests, handleCancel, handleStatusUpdate, setSelectedTrip }) => (
  <div style={S.card}>
    ...
    <div style={S.btnRow}>
      <button style={S.matchBtn} onClick={() => fetchRequests(trip._id)}>Find Companions</button>
      <button style={S.cancelBtn} onClick={() => handleCancel(trip._id)}>Cancel Trip</button>
    </div>
...
export default function TripPage() {
  ...
  const [showTripPayment, setShowTripPayment] = useState(false);
  const [tripPaymentData, setTripPaymentData] = useState(null);
  ...
  useEffect(() => {
    ...
    if (params.get("tab") === "plan") {
      setActiveTab("plan");
      if (savedPlan.destination) fetchRecommendations(savedPlan.destination);
    }
  }, [location]);
  ...
  const fetchDestSuggestions = async (search) => {
    ...
    try {
      ...
      const cities = []; const seen = new Set();
      data.forEach(p => { if (!seen.has(p.city)) { seen.add(p.city); cities.push({ city: p.city, category: p.category, name: p.name }); } });
      setDestSuggestions(cities); setShowDropdown(true);
    } catch (err) { console.error(err); }
  };
  ...
  const fetchRequests = async (tripId) => {
    try {
      ...
      setRequests(data); setSelectedTrip(tripId);
    } catch (err) { console.error(err); }
  };
  ...
  const handleJoin = async (tripId) => {
    ...
    try {
      ...
      fetchPendingPlans();
      // ✅ NO alert popup — just silently update state
    } catch (err) { alert(err.response?.data?.error || "Failed!"); }
  };

  const handleCancel = async (tripId) => {
    if (!window.confirm("Cancel this trip?")) return;
    try { await axios.delete(`${API}/${tripId}`); fetchTrips(); }
    catch (err) { console.error(err); }
  };

  // ✅ REMOVED: match buddy alert popup — just create the trip silently
  const handleSubmit = async () => {
    if (!form.destination || !form.startDate || !form.endDate || !form.budget) {
      alert("Please fill all required fields!"); return;
    }
    try {
      await axios.post(`${API}/create`, { ...form, userId: user._id });
      // ✅ No popup about matches — just redirect to My Trips
  ...
  // ── Trip payment success (card or COD) ──
  const handleTripPaymentSuccess = async (stripePaymentId, method = "card") => {
    try {
      await axios.post("http://localhost:5000/api/payment/confirm-trip", {
        matchId: tripPaymentData.matchId,
        stripePaymentId,
        userId:  user._id,
        amount:  tripPaymentData.amount,
        method,
      });
      setShowTripPayment(false);
      setPendingPlans(prev =>
        prev.map(r => r._id === tripPaymentData.matchId
          ? { ...r, paymentStatus: "paid", paymentMethod: method }
          : r
        )
      );
    } catch (err) { console.error(err); }
  ...
  return (
    <div style={S.wrapper}>
        ...
        {activeTab === "browse" && (
          <div>
            ...
            {loading ? <p style={S.loading}>Loading...</p> :
              ...
              <div style={S.grid}>
                {allTrips.map(t => <BrowseCard key={t._id} trip={t} joinedTrips={joinedTrips} handleJoin={handleJoin} />)}
              </div>
            }
          </div>
        )}
        ...
        {activeTab === "mine" && (
          <div>
            ...
            {myTrips.length === 0 ? (
              ...
              <div style={S.grid}>
                {myTrips.map(t => (
                  <MyTripCard key={t._id} trip={t}
                    selectedTrip={selectedTrip} requests={requests}
                    fetchRequests={fetchRequests} handleCancel={handleCancel}
                    handleStatusUpdate={handleStatusUpdate} setSelectedTrip={setSelectedTrip}
                  />
                ))}
              </div>
            )}
        ...
        {activeTab === "pending" && (
          <div>
            ...
            {pendingPlans.length === 0 ? (
              ...
              <div style={S.grid}>
                {pendingPlans.map(r => (
                  <PendingCard key={r._id} request={r} user={user}
                    setShowTripPayment={setShowTripPayment}
                    setTripPaymentData={setTripPaymentData}
                  />
                ))}
              </div>
            )}
              ...
              <div style={S.formRow}>
                <div style={S.formGroup}>
                  ...
                  <div style={{ position: "relative" }}>
                    <input style={S.input} placeholder="Search destination..."
                      value={form.destination}
                      onChange={e => { setForm({ ...form, destination: e.target.value }); fetchDestSuggestions(e.target.value); fetchRecommendations(e.target.value); }}
                      ...
                      <div style={S.dropdown}>
                        {destSuggestions.map((s, i) => (
                          <div key={i} style={S.dropdownItem}
                            onMouseDown={() => { setForm({ ...form, destination: s.city, category: s.category }); fetchRecommendations(s.city); setShowDropdown(false); localStorage.removeItem("planTrip"); }}>
                ...
                <div style={S.recGrid}>
                  {recommendations.map(p => (
                    <div key={p._id} style={S.recCard}>
                      {p.image ? <img src={p.image} alt={p.name} style={S.recImg} /> : <div style={S.recPlaceholder}>{p.name[0]}</div>}
      ...
      {/* Stripe / COD Payment */}
      {showTripPayment && tripPaymentData && (
        <StripePayment
          amount={tripPaymentData.amount}
          type="trip"
          onSuccess={handleTripPaymentSuccess}
          onCancel={() => setShowTripPayment(false)}
          metadata={{ type: "trip", matchId: tripPaymentData.matchId }}
        />
      )}
    </div>
  );
...
const S = {
  ...
  tabs:          { display: "flex", justifyContent: "center", marginBottom: 36, border: "1px solid #e2c4e2", borderRadius: 30, overflow: "hidden", maxWidth: 560, margin: "0 auto 36px" },
  ...
  receiptBtn:    { width: "100%", padding: "11px", background: "transparent", border: "1px solid #059669", color: "#059669", borderRadius: 10, cursor: "pointer", fontSize: 12, fontFamily: "sans-serif", letterSpacing: 1, marginTop: 8 },
  codTag:        { fontSize: 11, color: "#065f46", fontFamily: "sans-serif", background: "#d1fae5", padding: "3px 10px", borderRadius: 20, display: "inline-block", marginTop: 6, marginBottom: 6 },
